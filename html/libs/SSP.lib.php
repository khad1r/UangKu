...
class SSP
{
  ...
  static function data_output($columns, $data)
  {
    ...
    for ($i = 0, $ien = count($data); $i < $ien; $i++) {
      ...
      for ($j = 0, $jen = count($columns); $j < $jen; $j++) {
        $column = $columns[$j];
        $dataCol = $data[$i][$column['db']] ?? (isset($column['dbcol']) ? $data[$i][$column['dbcol']] : null);
        // Is there a formatter?
        if (isset($column['formatter'])) {
          $row[$column['dt']] = $column['formatter']($dataCol, $data[$i]);
        } else {
          $row[$column['dt']] = $dataCol;
        }
      }
  ...
  static function sql_exec($db, $bindings, $sql = null)
  {
    ...
    // Bind parameters
    if (is_array($bindings) && preg_match('/[:?]/', $sql)) {
