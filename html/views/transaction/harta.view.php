...
<script>
  ...
  const DT_TABLE = new DataTable(TABLE, {
      ...
      },
      {
        ...
        'data': function(data, type, dataToSet) {
          return /* HTML */ `
            ...
            <small class="kuitansi small font-italic">No./Ref. Sebenarnya : ${data.no_asli}</small><br>
            <small class="kuitansi small font-italic">Dibuat : ${data.tgl_dibuat}</small><br>
            <small class="kuitansi small font-italic">${(!data.aktif) ? 'Ditutup : '+data.tgl_ditutup : ''}</small>
          `
        }
      },
      {
        ...
        'data': function(data, type, dataToSet) {
          if (data.nominal_asing.length > 0) return /* HTML */ `
            <span class="nominal fw-bold text-primary">${(data.nominal_asing.length > 0) ? formattedNumber.format(data.saldo_asing)+' '+data.nominal_asing : ''}</span><br>
    ...
    initComplete: function() {
      const input = document.querySelector('#FormatTable_wrapper .dataTables_filter input');
